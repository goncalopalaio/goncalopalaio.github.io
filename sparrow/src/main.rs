...
fn read_posts(list: &Vec<PathBuf>) -> std::vec::Vec<Post> {
	...
	for p in &posts {
		println!("Post: {} | {} | {} | {}", p.name, p.title, p.date_str, p.date);
	}
...
fn get_md_files(prefix: &str) -> Vec<PathBuf> {
    return fs::read_dir("contents/").unwrap()
    .into_iter()
    .filter(|f| f.is_ok())
    .filter(|f| {
...
fn create_md_header(title: &str, pages: &Vec<PathBuf>) -> String {
	...
	for page in pages {
		...
		md_content.push_str(&format!("[{}]({}.html)", uppercase_first_letter(&page_name), &page_name))
	}
	md_content.push_str("\n\n\n---\n");
	return md_content;
}
