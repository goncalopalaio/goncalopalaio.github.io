...
I think this is a mostly unexplored topic since it’s pretty non-standard. I will mostly ignore the fact that you could add uiautomator or espresso to your project and perform your actions there. What I am interested is in solutions where it's not required that you make changes to your project.

I will try to make this as brief as I can so I will not delve into what each command argument means.
...
adb automator command #TODO

==========

This will print the view tree as xml and will allow you to look for the screen coordinates of the particular view that you want.

Another alternative is:

==========
adb shell getevent #TODO
==========


In case of getevent, you will get a log of the events that the device sensors are receiving, but you will notice that the values are in hexadecimal so you'll have to do that conversion first.
...
- ==TODO script name== will print the screencordinates as an integer. There's an alternative mode, but more on that in the following section.
...
It opens the file descriptor that represents the device sensor, sends a single event and then closes the file descriptor. See ==TODO Link to source==

To get around this, a few smart folks had the idea of recompiling sendevent so it would receive several events and writte in one shot to the device sensor file descriptor.
...
#TODO command to initiate a specific test in a test suite
This is meant to be a project where multiple tests exist.
